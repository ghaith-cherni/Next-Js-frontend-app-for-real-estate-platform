...
const PropertiesCards = ({
  ...
  return (
    <div
      className={`h-[467px] flex-1 rounded-3xs bg-gray-white border-whitesmoke-100 border-[1px] border-solid box-border flex flex-col items-start justify-start py-[15px] px-[13px] gap-[23px] min-w-[355px] max-w-[400px] text-left text-mini-5 text-gray-700 font-body-large-400 hover:bg-gainsboro hover:cursor-pointer sm:w-[100%!important] sm:mb-5 ${className}`}
      style={card1Style}
    >
      <img
      ...
      <div className="self-stretch flex flex-row items-center justify-between text-center text-xs-6">
        ...
        <div className="flex flex-row items-start justify-start gap-[8.7px]">
          <div className="rounded-[1.45px] bg-primary-50 flex flex-row items-start justify-start p-[2.9px]">
            <img
              className="w-[14.5px] relative h-[14.5px] object-cover"
              alt=""
              src={shareNetwork}
            />
          </div>
...
PropertiesCards.propTypes = {
  ...
  bathtub: PropTypes.string,
  ellipse1: PropTypes.string,
  shareNetwork: PropTypes.string,
